...
def image_molecules(context, topology):
    """Image molecules to put protein in center of box and ensure ligand is in closest image.

    Parameters
    ----------
    context : simtk.openmm.Context
        The context which will be modified
    ...
    with temporary_filename() as pdb_filename:
        with open(pdb_filename, 'wt') as outfile:
            app.PDBFile.writeFile(topology, state.getPositions(), outfile)

        mdtraj_trajectory = md.load(pdb_filename)
        mdtraj_trajectory.image_molecules(inplace=True)
        context.setPositions(mdtraj_trajectory[0].xyz * unit.nanometers)
...
def prepare_simulation(molecule, basedir, save_openmm=False):
    ...
    for phase in ['complex', 'ligand']:
        ...
        # Image the system into central periodic box
        image_molecules(context, modeller.topology)
